...
{
    public class AddAddressingComponent : ComponentBase
    {
        ...
        {
            var reestrDbGetStreetsResult = await ReestrDb.StreetsUnitOfWork.Get();
            getStreetsResult = reestrDbGetStreetsResult;

            getStreetCategoriesResult = await ReestrDb.StreetCategoryUnitOfWork.Get();
            getStreetCategoriesResult = getStreetCategoriesResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Addressing args)
        {
            try
            {
                var reestrDbCreateAddressingResult = await AddressingS.CreateAddressing(addressing);
                DialogService.Close(addressing);
            }
            catch (System.Exception reestrDbCreateAddressingException)
            {
                NotificationService.Notify(new NotificationMessage() { Severity = NotificationSeverity.Error, Summary = $"Error", Detail = $"Unable to create new Addressing!" });
            }
        }
